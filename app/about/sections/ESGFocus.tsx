...
export function ESGFocus() {
  return (
    <Section className="bg-ink-900 border-t border-line">
      <Container>
        <p className="font-mono text-xs uppercase tracking-wider font-medium text-gold mb-4">(06) SUSTAINABILITY &amp; ESG</p>
        <h2 className="font-display text-[clamp(30px,5.5vw,45px)] uppercase leading-[0.95] mb-4">ROOTED IN ETHICS. GROWING WITH TRUST.</h2>
        <p className="font-body text-[clamp(20px,1.6vw,28px)] leading-tight text-bone-dim mb-16 max-w-[55ch]">
  ...
  return (
    <Grid cols={2} className="gap-12 items-center">
      <div className={isEven ? "order-1" : "order-1 lg:order-2"}>
        <span className="font-mono text-xs uppercase tracking-wider font-medium text-gold font-bold">{panel.num}</span>
        <h3 className="font-display text-[clamp(30px,5.5vw,45px)] uppercase leading-[0.95] text-bone mt-2 mb-4">{panel.title}</h3>
        <p className="font-body text-[clamp(20px,1.6vw,28px)] leading-tight text-bone-dim">{panel.subtitle}</p>
      </div>
